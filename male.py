...
pygame.display.update() #updates display, framerate(uses seen later)
clock = pygame.time.Clock() #defines the fps for a program, creates frame rates

selected_family = "black"

# basis for creating a piece
class piece:
    x = 0  # x coordinate
    y = 0  # y coordinate
...
#the list stores the data of a piece position, what type and the family where it belongs
...
#gives images to pieces using the data stored in the pie list
    ...
    while i < len(pie):
        if pie[i].rank == "p" and pie[i].life:
            ...
            if pie[i].family == "white":
                img = pygame.image.load("C:/Users/ADMIN/Desktop/minu male/uued pildid/pawnwhite.png")
            else:
                img = pygame.image.load("C:/Users/ADMIN/Desktop/minu male/uued pildid/pawnblack.png")
            game_display.blit(img, ((2 * pie[i].x) * factor, ((2 * pie[i].y) * factor)))

        elif pie[i].rank == "q" and pie[i].life:

            if pie[i].family == "white":
                img = pygame.image.load("C:/Users/ADMIN/Desktop/minu male/uued pildid/queenwhite.png")
            else:
                img = pygame.image.load("C:/Users/ADMIN/Desktop/minu male/uued pildid/queenblack.png")
            game_display.blit(img, ((2 * pie[i].x) * factor, ((2 * pie[i].y) * factor)))
        elif pie[i].rank == "b" and pie[i].life:

            if pie[i].family == "white":
                img = pygame.image.load("C:/Users/ADMIN/Desktop/minu male/uued pildid/valgeoda.png")
            else:
                img = pygame.image.load("C:/Users/ADMIN/Desktop/minu male/uued pildid/mustoda.png")
            game_display.blit(img, ((2 * pie[i].x) * factor, ((2 * pie[i].y) * factor)))
        elif pie[i].rank == "r" and pie[i].life:

            if pie[i].family == "white":
                img = pygame.image.load("C:/Users/ADMIN/Desktop/minu male/uued pildid/kahurvalge.png")
            else:
                img = pygame.image.load("C:/Users/ADMIN/Desktop/minu male/uued pildid/kahurmust.png")
            game_display.blit(img, ((2 * pie[i].x) * factor, ((2 * pie[i].y) * factor)))
        elif pie[i].rank == "k" and pie[i].life:

            if pie[i].family == "white":
                img = pygame.image.load("C:/Users/ADMIN/Desktop/minu male/uued pildid/hobunevalge.png")
            else:
                img = pygame.image.load("C:/Users/ADMIN/Desktop/minu male/uued pildid/hobunemust.png")
            game_display.blit(img, ((2 * pie[i].x) * factor, ((2 * pie[i].y) * factor)))
        elif pie[i].rank == "king" and pie[i].life:

            if pie[i].family == "white":
                img = pygame.image.load("C:/Users/ADMIN/Desktop/minu male/uued pildid/kuningasvalge.png")
            else:
                img = pygame.image.load("C:/Users/ADMIN/Desktop/minu male/uued pildid/kuningasmust.png")
            game_display.blit(img, ((2 * pie[i].x) * factor, ((2 * pie[i].y) * factor)))
        i += 1

#! test theory
#! another way to make the board which failed
#! acts as a buffer for the turn bug
def clear():
    i = 0
    while i < len(pie):
...
def move(orignal_x, orignal_y, final_x, final_y):
    ...
    fam = selected_family
    for i in range(len(pie)): #finds out if an original piece is valid (is alive, correct original coordinates, etc)
        final_pie = None
        if pie[i].x == orignal_x and pie[i].y == orignal_y and pie[i].life and pie[i].family == fam:
            for k in range(len(pie)): # if you click on the same piece twice then function breaks
                if pie[k].x == final_x and pie[k].y == final_y and pie[k].life:
                    final_pie = pie[k]
                    t = k
                    break
                    # If the pieces are not of same family then
            # THe killing of second click piece
...
def game():
    ...
    while True:
        ...
        myfont = pygame.font.Font("C:/Users/ADMIN/Desktop/minu male/male font/OpenSans-Italic.ttf", 30)
        if selected_family == "black":
            string = "Musta kord"
        else:
            string = "Valge kord"
